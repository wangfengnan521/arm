...
class GravityTeach(Node):
    def __init__(self) -> None:
        ...
        self.joy_pub = self.create_publisher(Int32MultiArray, "/arx_joy", 10)
        ...
        self.get_logger().info(
            f"HOLD publishes {self.cmd_topic} at {self.rate_hz:.0f} Hz; "
            "gravity uses one-shot /arx_joy"
        )
    ...
    def on_timer(self) -> None:
        # Gravity compensation is a latched controller state. Re-sending the
        # transition continuously can repeatedly reset the compensator and
        # make the arm feel heavily damped. Only HOLD needs continuous targets.
        if self.mode != MODE_G_COMPENSATION:
            self._publish_cmd()

    def _publish_gravity(self) -> None:
        msg = Int32MultiArray()
        msg.data = [1, 0]
    ...
    def set_gravity(self) -> None:
        self.mode = MODE_G_COMPENSATION
        # Use X5Controller's state-only callback. RobotCmd(mode=3) first writes
        # joint targets and makes this EC-based arm feel like damping mode.
        deadline = time.time() + 3.0
        while (
            (self.q is None or self.joy_pub.get_subscription_count() == 0)
            and time.time() < deadline
        ):
            rclpy.spin_once(self, timeout_sec=0.05)
        if self.q is None:
            self.get_logger().error("cannot enter gravity: no /arm_status")
            return
        if self.joy_pub.get_subscription_count() == 0:
            self.get_logger().error("cannot enter gravity: no /arx_joy subscriber")
            return
        self._publish_gravity()
    ...
    def restore_mode(self, mode: int) -> None:
        """Restore the hardware controller state after a sample attempt."""
        self.mode = int(mode)
        if self.mode == MODE_G_COMPENSATION:
            self._publish_gravity()
            self.get_logger().info("restored GRAVITY mode")
        else:
            self._publish_cmd()
...
def main(args=None):
    ...
    try:
        tty.setcbreak(fd)
        print("\n=== X5A Gravity Teach ===")
        print("startup is monitor-only; arm must already be in gravity mode")
